...
public class EventHandler implements OnClickListener {
	...
	int back_button, home_button, info_button, help_button, manage_button;
	int multiselect_button, hidden_attach, hidden_move, hidden_copy, hidden_delete;
	int hidden_buttons;
	
	int pdf, music, image, zip, movies, word, excel, ppt, html32, xml32, config32, appicon, jar32, text, folder_full, folder, toolbox;
	int tablerow;
	...
	public EventHandler(Context context, final FileManager manager) {
		...
		tablerow = mContext.getResources().getIdentifier("manager_tablerow", "layout", mContext.getPackageName());
		
		back_button = context.getResources().getIdentifier("back_button", "id", context.getPackageName());
		home_button = context.getResources().getIdentifier("home_button", "id", context.getPackageName());
		info_button = context.getResources().getIdentifier("info_button", "id", context.getPackageName());
		help_button = context.getResources().getIdentifier("help_button", "id", context.getPackageName());
		manage_button = context.getResources().getIdentifier("manage_button", "id", context.getPackageName());
		multiselect_button = context.getResources().getIdentifier("multiselect_button", "id", context.getPackageName());
		hidden_attach = context.getResources().getIdentifier("hidden_attach", "id", context.getPackageName());
		hidden_move = context.getResources().getIdentifier("hidden_move", "id", context.getPackageName());
		hidden_copy = context.getResources().getIdentifier("hidden_copy", "id", context.getPackageName());
		hidden_delete = context.getResources().getIdentifier("hidden_delete", "id", context.getPackageName());
		hidden_buttons = context.getResources().getIdentifier("hidden_buttons", "id", context.getPackageName());
		
		pdf = context.getResources().getIdentifier("pdf", "drawable", context.getPackageName());
		music = context.getResources().getIdentifier("music", "drawable", context.getPackageName());
		image = context.getResources().getIdentifier("image", "drawable", context.getPackageName());
		zip = context.getResources().getIdentifier("zip", "drawable", context.getPackageName());
		movies = context.getResources().getIdentifier("movies", "drawable", context.getPackageName());
		word = context.getResources().getIdentifier("word", "drawable", context.getPackageName());
		excel = context.getResources().getIdentifier("excel", "drawable", context.getPackageName());
		ppt = context.getResources().getIdentifier("ppt", "drawable", context.getPackageName());
		html32 = context.getResources().getIdentifier("html32", "drawable", context.getPackageName());
		xml32 = context.getResources().getIdentifier("xml32", "drawable", context.getPackageName());
		config32 = context.getResources().getIdentifier("config32", "drawable", context.getPackageName());
		appicon = context.getResources().getIdentifier("appicon", "drawable", context.getPackageName());
		jar32 = context.getResources().getIdentifier("jar32", "drawable", context.getPackageName());
		text = context.getResources().getIdentifier("text", "drawable", context.getPackageName());
		folder_full = context.getResources().getIdentifier("folder_full", "drawable", context.getPackageName());
		folder = context.getResources().getIdentifier("folder", "drawable", context.getPackageName());
		toolbox = context.getResources().getIdentifier("toolbox", "drawable", context.getPackageName());
	...
	public void onClick(View v) {
		...
		if (id == back_button) {
		...
		} else if (id == manage_button) {
			display_dialog(MANAGE_DIALOG);
		} else if (id == multiselect_button) {
			if(multi_select_flag) {
				mDelegate.killMultiSelect(true);				
				
			} else {
				LinearLayout hidden_lay = 
					(LinearLayout)((Activity) mContext).findViewById(hidden_buttons);
				
				multi_select_flag = true;
				hidden_lay.setVisibility(LinearLayout.VISIBLE);
			}
		} else if (id == hidden_attach) {
			...
			intent.putExtra(Constants.KEY_FILE_SELECTED, uris.toString());
			((Activity) mContext).setResult(Activity.RESULT_OK, intent);
			Log.i("FILE CHOOSER", "result ok");
			((Activity) mContext).finish();
			mDelegate.killMultiSelect(true);
		} else if (id == hidden_move || id == hidden_copy) {
			/* check if user selected objects before going further */
			if(mMultiSelectData == null || mMultiSelectData.isEmpty()) {
				mDelegate.killMultiSelect(true);
				return;
			}
			if(v.getId() == hidden_move)
				delete_after_copy = true;
			mInfoLabel.setText("Holding " + mMultiSelectData.size() + 
							   " file(s)");
			mDelegate.killMultiSelect(false);
		} else if (id == hidden_delete) {
			/* check if user selected objects before going further */
			if(mMultiSelectData == null || mMultiSelectData.isEmpty()) {
				mDelegate.killMultiSelect(true);
				return;
    ...
    public class TableRow extends ArrayAdapter<String> {
    	...
    	public TableRow() {
    		...
    		super(mContext, tablerow, mDataSource);    		
    	}
    	...
    	public void killMultiSelect(boolean clearData) {
    		hidden_layout = (LinearLayout)((Activity)mContext).findViewById(hidden_buttons);
    	...
    	public View getView(int position, View convertView, ViewGroup parent) {
    		...
    		int top_view = mContext.getResources().getIdentifier("top_view", "id", mContext.getPackageName());
    		int bottom_view = mContext.getResources().getIdentifier("bottom_view", "id", mContext.getPackageName());
    		int row_image = mContext.getResources().getIdentifier("row_image", "id", mContext.getPackageName());
    		int multiselect_icon = mContext.getResources().getIdentifier("multiselect_icon", "id", mContext.getPackageName());
    		int singlecheck = mContext.getResources().getIdentifier("singlecheck", "drawable", mContext.getPackageName());
    		...
    		if(convertView == null) {
    			...
    			mViewHolder.topView = (TextView)convertView.findViewById(top_view);
    			mViewHolder.bottomView = (TextView)convertView.findViewById(bottom_view);
    			mViewHolder.icon = (ImageView)convertView.findViewById(row_image);
    			mViewHolder.mSelect = (ImageView)convertView.findViewById(multiselect_icon);
				mViewHolder.mSelect.setImageResource(singlecheck);
    		...
    		if(file != null && file.isFile()) {
    			...
    			if (sub_ext.equalsIgnoreCase("pdf")) {
    				mViewHolder.icon.setImageResource(pdf);
    				...
    				mViewHolder.icon.setImageResource(music);
    				...
    				if(thumbnail_flag && file.length() != 0) {
    					...
    					mViewHolder.icon.setImageResource(image);
    				}
    				...
    				mViewHolder.icon.setImageResource(zip);
    				...
    				mViewHolder.icon.setImageResource(movies);
    			
    			} else if(sub_ext.equalsIgnoreCase("doc") || 
    					  sub_ext.equalsIgnoreCase("docx")) {
    				
    				mViewHolder.icon.setImageResource(word);
    			
    			} else if(sub_ext.equalsIgnoreCase("xls") || 
    					  sub_ext.equalsIgnoreCase("xlsx")) {
    				
    				mViewHolder.icon.setImageResource(excel);
    				
    			} else if(sub_ext.equalsIgnoreCase("ppt") ||
    					  sub_ext.equalsIgnoreCase("pptx")) {
    				
    				mViewHolder.icon.setImageResource(ppt);   	
    				
    			} else if(sub_ext.equalsIgnoreCase("html")) {
    				mViewHolder.icon.setImageResource(html32);  
    				
    			} else if(sub_ext.equalsIgnoreCase("xml")) {
    				mViewHolder.icon.setImageResource(xml32);
    				
    			} else if(sub_ext.equalsIgnoreCase("conf")) {
    				mViewHolder.icon.setImageResource(config32);
    				
    			} else if(sub_ext.equalsIgnoreCase("apk")) {
    				mViewHolder.icon.setImageResource(appicon);
    				
    			} else if(sub_ext.equalsIgnoreCase("jar")) {
    				mViewHolder.icon.setImageResource(jar32);
    				
    			} else {
    				mViewHolder.icon.setImageResource(text);
    			}
    			
    		} else if (file != null && file.isDirectory()) {
    			if (file.canRead() && file.list().length > 0)
    				mViewHolder.icon.setImageResource(folder_full);
    			else
    				mViewHolder.icon.setImageResource(folder);
    		}
